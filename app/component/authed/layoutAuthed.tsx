...
export default function LayoutAuthed({children,} : {children : React.ReactNode}) {
    ...
    return (
        <section className="bg-gray-100 flex">
            ...
            <div className={`border-r-[0.5px] ${isOpen ? 'w-1/24' : 'w-1/6'} transition-all duration-300 border-r-gray-300 h-screen flex flex-col sticky top-0 left-0 justify-between`}>
                <div className="flex items-center justify-center h-18 border-b-[0.5px] border-b-gray-300 overflow-hidden">
                    <h1 className="text-center italic text-blue-700 font-semibold">Practine</h1>
                </div>
                ...
                <ul className={`flex flex-col gap-2 px-3 ${isOpen ? 'items-center' : ''}`}>
                    ...
                    <li onClick={() => router.push('/authed/stock')} className="py-2 px-3 text-gray-600 tracking-wide rounded-y rounded-r hover:bg-gray-200 transition duration-200 cursor-pointer flex gap-2 items-center"> <i className={`text-center text-gray-600 ${isOpen ? 'px-4 py-2' : 'w-6'} fa fa-cubes`} aria-hidden="true"></i> <p hidden={isOpen}>Stok</p></li>
                ...
                <div onClick={() => setIsOpen(!isOpen)} className={`border-t-[0.5px] border-t-gray-300 flex ${isOpen ? 'justify-center' : 'justify-end'} items-center h-16 p-2`}>
            ...
            <div className="w-full">
                <div className=" sticky top-0 z-50 bg-gray-100 h-18 border-b-[0.5px] border-b-gray-300 w-full flex justify-end p-4">
